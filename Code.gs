...
function renameWallet(id, name) {
  const s = SS().getSheetByName('_wallets'), data = s.getDataRange().getValues();
  for (let i=1; i<data.length; i++) {
    if (String(data[i][0])===String(id)) { s.getRange(i+1,2).setValue(name); return; }
  }
}
function deleteWallet(id) {
  deleteRowById('_wallets', id);
  cascadeDelete('_transactions', 1, id);
  cascadeDelete('_cashflows', 1, id);
}

// ── 交易 ──────────────────────────────────────
function getTransactions(walletId) {
  return sheetToObjects('_transactions')
    .filter(r => String(r.walletId)===String(walletId))
    .map(r => ({
      ...r,
...
function getCashFlows(walletId) {
  return sheetToObjects('_cashflows')
    .filter(r => String(r.walletId)===String(walletId))
    .map(r => ({ ...r, amount: parseFloat(r.amount)||0 }));
}

// 修正：appendRow 的欄位數必須對應 schema ['id','walletId','type','amount','note','isDividend','date']
function addCashFlow(data) {
  const id = newId();
  SS().getSheetByName('_cashflows').appendRow([
    id, data.walletId, data.type, data.amount, data.note||'',
    data.isDividend||'', data.date
  ]);
  return id;
}
...
function updatePriceSheet(symbol, type) {
  ...
  s.getRange(row,1).setValue(symbol);
  s.getRange(row,2).setValue(type);
  s.getRange(row,3).setFormula(formula);
}
...
function getPrices() {
  ...
  data.slice(1).forEach(r => {
    if (r[0] && typeof r[2]==='number' && r[2]>0) result[String(r[0])] = r[2];
  });
  return result;
}
...
function refreshPrices() {
  const s = SS().getSheetByName('_prices');
  if (!s || s.getLastRow()<=1) return { prices: {}, timestamp: null, source: 'Google Finance' };
  ...
  if (lastUpdate) {
    const elapsed = (now - parseInt(lastUpdate)) / 1000;
    if (elapsed < 300) {
  ...
  const formulas = s.getRange(2, 3, lastRow-1, 1).getFormulas();
...
function _lookupGF(gSym, displaySymbol, type, currency) {
  const ss = SS();
  let temp = ss.getSheetByName('_temp');
  ...
  try {
    ...
    const name  = temp.getRange('A1').getValue();
    const price = temp.getRange('B1').getValue();
    temp.getRange('A1:B1').clearContent();
    if (name && typeof name==='string' && name.length>1 && !name.startsWith('#'))
      return { symbol: displaySymbol, name, type, currency, price: (typeof price==='number' && price>0) ? price : null };
    return null;
  } catch(e) {
    try { temp.getRange('A1:B1').clearContent(); } catch(_) {}
    return null;
  }
}
...
function loadWalletData(walletId) {
  return {
    transactions: getTransactions(walletId),
    cashFlows:    getCashFlows(walletId),
    prices:       getPrices()
  };
}
...
function deleteRowById(sheetName, id) {
  ...
  for (let i=data.length-1; i>=1; i--) {
    if (String(data[i][0])===String(id)) { s.deleteRow(i+1); return; }
  }
}

function cascadeDelete(sheetName, fkCol, fkValue) {
  const s = SS().getSheetByName(sheetName);
  if (!s || s.getLastRow()<=1) return;
  const data = s.getDataRange().getValues();
  for (let i=data.length-1; i>=1; i--) {
    if (String(data[i][fkCol])===String(fkValue)) s.deleteRow(i+1);
  }
}
